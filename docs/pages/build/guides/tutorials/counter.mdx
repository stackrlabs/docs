...
{/* prettier-ignore */}
```ts [transitions.ts]
import { REQUIRE, SolidityType, STF, Transitions } from "@stackr/sdk/machine"; // [!code focus]
...
const increment: STF<CounterState, typeof schema> = { // [!code focus]
  schema,
...
const decrement: STF<CounterState, typeof schema> = { // [!code focus]
  schema,
  handler: ({ state }) => { // [!code focus]
    state -= 1; // [!code focus]
...
{/* prettier-ignore */}
```ts [transitions.ts]
import { REQUIRE, SolidityType, STF, Transitions } from "@stackr/sdk/machine";
...
Next we sign the inputs using the [`wallet.signTypedData`](https://docs.ethers.org/v6/api/providers/#Signer-signTypedData) function. This uses `EIP-712` domain and types definitions to sign the message.
...
const signature = await wallet.signTypedData(domain, types, inputs); // [!code focus]
```
...
const domain = mru.config.domain;
const types = mru.getStfSchemaMap()["increment"];
const signature = await wallet.signTypedData(domain, types, inputs); 
const incrementActionParams = { // [!code focus]
  name: "increment", // [!code focus]
  inputs, // [!code focus]
