...
If you are getting `trace trap` error when using Bun. This is a known issue with Bun and SQLite, which got introduced in `v1.1.27` of bun. They've fixed the issue in `v1.1.28`. [More details here](https://discord.com/channels/876711213126520882/1282959007589601280/1283307017158791222).

Moving forward we recommend using `tsx` over `bun` for development.
If you want to continue using `bun`, you can upgrade its version by running:

```bash
bun upgrade
bun --version # check that you are on >= v1.1.28
```
