...
You can also set `allowEmptyBlocks` to `true` if you want your micro-rollup to always produce blocks (even when there are no actions).
  ```ts [stackr.config.ts]
  sequencer: {
    blockSize: 16,
    blockTime: 10,
...
- The `domain` in the config is used to create EIP-712 Typed Signature over the inputs of actions. Feel free to tune this as per your requirements.
  ...
  ```
- You can refer to [Stackr Config](/build/framework/config) section on explanation of each and every available configuration property.
...
- `actions.ts` contains the [Action Schemas](/build/framework/action/action-schema) for your application.
