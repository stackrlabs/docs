...
- Completely remove the need for `ActionSchema`. Input schemas are now [defined directly inside the STFs](/build/framework/state-machine/state-transitions#passing-inputs-to-an-stf). See [upgrade guide](/build/upgrade-guide).
- The [`getOrderedActions`](/build/framework/sequencer#custom-ordering-strategy) method of a sequencer strategy now accepts list of actions as a `Readonly<AcknowledgedAction[]>` type
and is supposed to only return the ordered list of action hashes as opposed to `Action` objects earlier.
