...
- [SDK] Perform C2, C3A, C3B syncs for stale blocks on MRU start-up.
- [SDK] Add `previousState` `previousStateRoot` fields to `ExecutorEvents.EXECUTE_BLOCK` event payload ([`ExecuteBlockEventArgs`](/build/framework/events/#executoreventsexecute_block)).
