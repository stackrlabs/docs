...
- Remove `domain.chainId` and `domain.verifyingContracts` fields.
2. If you are passing any of `isSandbox`, `disableBuilder`, `disableVulcanSync` or `disableL1Sync` arguments to `MicroRollup`, they should be moved to the config now.
