...
### `ack.waitFor(actionConfirmationStatus?: ActionConfirmationStatus)`
...
- `actionConfirmationStatus`: Confirmation status to wait for (defaults to C1).
...
### `actions.waitFor(actionHash: Keccak256, actionConfirmationStatus = ActionConfirmationStatus.C1)`
...
Returns the execution logs for the action. If not null, the execution succeeded without any errors. It is null if the action has not yet been executed.

**Returns:** `ExecutionLog[] | null`

### `action.errors`

Returns any errors that occurred during the execution of the action. It is null if the action has not yet been executed.
