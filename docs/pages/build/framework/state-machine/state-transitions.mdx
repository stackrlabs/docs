...
```typescript
import { SolidityType, STF, Transitions } from "@stackr/sdk/machine";
...
const superCoolStf: STF<MachineState, typeof schema> = {
  schema,
  handler: (props) => {
    const state = props.
    //                  ^|
...
// With Destructuring
const anotherSuperCoolStf: STF<MachineState, typeof schema> = {
  schema,
  handler: ({ s }) => {
    //         ^|
  },
