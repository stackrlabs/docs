...
The current block height, timestamp and parent block's hash are passed as arguments to the state transition function. These can be used to generate pseudo-random numbers or to implement time-based logic.
