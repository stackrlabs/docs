...
- `enableEmptyBlocks`: A flag to allow creation of empty blocks (the one with no actions). Defauts to `false`.
- `disableBuilder`: A flag to indicate if the rollup should not use the builder to generate new blocks. This is useful for [functionally generating blocks](/build/framework/micro-rollup-utils#sequencertick) when needed. Defauts to `false`.
