...
The third level of confirmation involves the transmission of the action batch to the verification layer, which, pessimistically re-executes the state transition for correctness, thus cementing the assurance that the application has properly executed the state transitions.
