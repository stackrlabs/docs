...
Get started by running the following commands:
  ...
  3. Modify values in stackr.config.ts & .env
  4. npx @stackr/cli@latest register
  5. Implement your State Machines
  6. npx @stackr/cli@latest deploy
...
- Update the `PRIVATE_KEY` in the `.env` file with your the Private Key of the account you used to register for the Beta Access.
...
- Cross check these values from the latest values listed in the [Network Information] Page.
    - `registryContract`
    - `syncer.L1RPC`
    - `syncer.vulcanRPC`
...
- The `src` directory contains the code for the Micro-rollup or your application.
...
- `stf.ts` contains the `STF` (State Transition Functions) for your application.

:::warning
  For the time of beta, it is recommended that the `machine.ts` or the file containing the StateMachine, doesn't import anything from `@stackr/sdk` instead uses `@stackr/sdk/machine` for the `StateMachine`, `State`, `Transitions` and other utility classes.
:::
